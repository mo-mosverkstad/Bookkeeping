...
// SDL2 render backend — draws to an SDL window.
struct SDL2Backend : RenderBackend {
    SDL_Window* window;
    SDL_Renderer* renderer;
    int win_w, win_h;

    SDL2Backend(const char* title, int w, int h) : win_w(w), win_h(h) {
    ...
    void render_ellipse(float abs_x, float abs_y, const Ellipse& e) override {
        // Midpoint ellipse (draw filled scanlines)
    ...
    void render_polyline(float abs_x, float abs_y, const Polyline& p) override {
        ...
        for (uint16_t i = 0; i + 1 < p.count; i++) {
            SDL_RenderDrawLine(renderer,
                (int)(abs_x + p.points[i].x), (int)(abs_y + p.points[i].y),
    ...
    void render_text(float abs_x, float abs_y, const Text& t) override {
        // Placeholder: draw a tinted rect where text would be.
        // Full text requires SDL2_ttf integration (Phase 1 extension).
        SDL_Rect rect = {(int)(abs_x + t.x), (int)(abs_y + t.y),
                         t.max_width > 0 ? (int)t.max_width : 80, (int)t.size};
